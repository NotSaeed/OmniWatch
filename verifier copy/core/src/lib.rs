...
pub mod rules {
    ...
    /// Sprint 4: Modbus write operation detected (FC 5, 6, 15, 16)
    pub const MODBUS_WRITE: u32      = 1 << 8;
    /// Sprint 4: Invalid/unusual Modbus function code (> 127 = exception responses)
...
///
/// Sprint 4 additions: Modbus-specific fields for ICS/SCADA monitoring.
/// `modbus_func_code`  — FC 03 (read), FC 05 (write coil), FC 16 (write regs), etc.
/// `modbus_unit_id`    — target PLC unit ID on the Modbus bus.
/// `zeek_uid`          — unique Zeek conn_uid (18 bytes, zero-padded).
/// `epoch_nonce`       — process-boot timestamp to prevent cross-session collisions.
...
pub struct NetworkTelemetry {
    ...
    // ── Sprint 4: Modbus ICS/SCADA fields ────────────────────────────────
    /// Modbus function code: 0=N/A, 1=Read Coils, 3=Read Holding Registers,
    /// 5=Write Single Coil, 6=Write Single Register, 16=Write Multiple Registers.
    /// Non-zero values trigger ICS-specific threat rules.
    pub modbus_func_code: u8,
    /// Modbus unit/slave ID on the bus (0–255). 0 = not applicable.
    pub modbus_unit_id: u8,
    /// Zeek conn_uid — unique per-connection identifier (18 bytes, zero-padded).
    /// Committed to the STARK journal hash for replay prevention.
    pub zeek_uid: [u8; 18],
    /// Process-boot epoch timestamp (seconds since UNIX epoch).
    /// Combined with zeek_uid to prevent cross-session nonce collisions.
    pub epoch_nonce: u64,
}
